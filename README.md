...
The **AI FAQ Assistant API** is a robust RESTful backend application designed to enable users to create and manage FAQs while leveraging Google Gemini AI (`gemini-3.6-flash`) to generate answers to arbitrary user questions and automatically generate structured FAQ question-and-answer pairs.
...
3.  **AI Answer Generator (Google Gemini)**:
    *   **Answer Endpoint**: Provides direct, concise answers using the `gemini-3.6-flash` model.
...
# Optional: comma-separated DNS servers when the local resolver cannot resolve MongoDB SRV records
