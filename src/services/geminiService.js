...
const FAQ_MODELS = ['gemini-3.6-flash', 'gemini-3.6-flash-lite'];
const RETRY_DELAY_MS = 750;

const isTransientGeminiError = (error) => {
  const details = `${error?.status || ''} ${error?.code || ''} ${error?.message || ''}`;
  return /503|UNAVAILABLE|high demand/i.test(details);
};
    ...
    for (const model of FAQ_MODELS) {
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          lastError = error;
          if (!isTransientGeminiError(error) || attempt === 1) {
            break;
          }
